...
#     top_indices = predictions[0].argsort()[-1:][::-1]  # Get indices of the top 3 predictions

#     # Get the class names and probabilities for the top 3 predictions
#     top_labels = [class_labels.get(idx, 'Unknown') for idx in top_indices]
...
#                 top_labels, top_probs = predict_image(cropped_image, model)
...
def predict_image(img, model):
    ...
    predictions = model.predict(img_array)  # Predict class probabilities
    top_indices = predictions[0].argsort()[-1:][::-1]  # Get index of the top prediction

    # Get the class name for the top prediction
    top_label = class_labels.get(top_indices[0], 'Unknown')
    top_prob = predictions[0][top_indices[0]]

    return top_label, top_prob

# Function to map predictions to suggested classes
def get_suggested_classes(top_label):
    suggestions = {
        'A1': ['B1', 'B2'],
        'B2': ['A1', 'D2'],
        'D2': ['B2', 'A2'],
    ...
    return suggestions.get(top_label, ['Unknown'])
...
if uploaded_file is not None:
    ...
    if st.button("Predict") and model is not None:
        if cropped_image is None:
            ...
            with st.spinner("Predicting..."):
                top_label, top_prob = predict_image(cropped_image, model)
                st.success("Prediction complete!")
                ...
                # # Display bar chart
                # fig, ax = plt.subplots()
                # ax.barh([top_label], [top_prob], color='skyblue')
                # ax.set_xlabel('Probability')
                # ax.set_xlim(0, 1)
